...
def main(args=None):
    ...
    if args.mode == "train":
        ...
        for fold in range(args.num_folds):
            ...
            loaders_for_reverse_diffusion = get_data(
                data, fold, args, for_reverse_diffusion=True
            )
            printt("finished creating data splits")
            # get model and load checkpoint, if relevant
            model = load_model_for_training(
                args, data_params, fold, load_best=True
            )  # load last_model to continue training
