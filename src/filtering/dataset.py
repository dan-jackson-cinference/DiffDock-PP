...
from config import ConfidenceModelCfg

from geom_utils import set_time

RMSD = {"complex": "crmsds", "interface": "irmsds", "simple": "rmsds"}


def deserialize_batch(
    batch_directory,
    duplicate_rec_and_esm=True,
    rmsd_type: Literal["complex", "interface", "simple"]
):
    ...
    rmsd_multiple_iterations = torch.load(f"{batch_directory}/{RMSD[rmsd_type]}.pkl")
...
def deserialize_batch_into_single_list(
    batch_directory, rmsd_type: Literal["complex", "interface", "simple"]
):
    complexes, ligand_positions, rmsds = deserialize_batch(
        batch_directory,
        duplicate_rec_and_esm=False, rmsd_type
    )

    return complexes, sum(ligand_positions, []), sum(rmsds, [])  # Concat samples


class ConfidenceBindingDataset(Dataset):
    """
    ...
    def __init__(
        ...
        super(ConfidenceBindingDataset, self).__init__()
...
class ConfidenceBindingDiskLoader(Dataset):
    """
    Protein-protein binding dataset
    """
    ...
    def __init__(
        ...
        super(ConfidenceBindingDiskLoader, self).__init__()
...
def get_confidence_loader(split: str, cfg: ConfidenceModelCfg, batch: Optional[str]=None, batch_size: int = 128, num_workers:int = 0) -> DataLoader:
    samples = ConfidenceBindingDataset(
        split,
        cfg.samples_directory,
        batch=batch,
        rmsd_type = cfg.rmsd_type)
...
def get_confidence_disc_loader(split, args, shuffle):
    samples = ConfidenceBindingDiskLoader(
